...
import { Activity, Flag, Home, List, User } from 'lucide-react'
import { useFinance } from './FinanceProvider'
import { useTranslation } from './useTranslation'

const navItems = [
  { labelKey: 'nav.home', href: '/', badgeKey: 'nav.dashboard', Icon: Home },
  { labelKey: 'nav.history', href: '/history', badgeKey: 'nav.transactions', Icon: List },
  { labelKey: 'nav.stats', href: '/stats', badgeKey: 'nav.insights', Icon: Activity },
  { labelKey: 'nav.goals', href: '/goals', badgeKey: 'nav.goals', Icon: Flag },
  { labelKey: 'nav.profile', href: '/profile', badgeKey: 'nav.account', Icon: User },
]
...
export default function NavigationLayout({ title, subtitle, children }: NavigationLayoutProps) {
  ...
  return (
    <div className="finleaf-shell min-h-screen text-slate-900 dark:text-slate-100">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 overflow-hidden border-t border-slate-200 bg-white/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 shadow-2xl shadow-slate-900/10 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/95 lg:hidden">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex min-h-14 flex-col items-center justify-center rounded-2xl px-1 py-3 text-[12px] font-semibold transition ${
